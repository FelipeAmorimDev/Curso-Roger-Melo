...
div.addEventListener('click', event => {
  ...
  if (HTMLElement.tagName === 'P') {
    secondaryTitle.innerText = `Clicou no ${HTMLElement.tagName}, filho da div.`
  } else if (HTMLElement.tagName === 'DIV') {
    secondaryTitle.innerText = 'Clicou na div.'
  }
...
secondaryTitle.addEventListener('copy', () => {
...
eggElement.addEventListener('mousemove', event => {
  const cordX = event.offsetX;
  const cordY = event.offsetY;
  eggElement.innerHTML = `Eixo X: ${cordX} | Eixo Y: ${cordY}`
})
...
isAnyFrontendDeveloper = people.some( element => element.profession === 'Front-end developer')
