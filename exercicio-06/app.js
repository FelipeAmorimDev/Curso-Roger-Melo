...
*/
  ...
  const age = 27;
  let hasAgeForVoucher = age <= 7 || age >= 65
  let mensagem;
  if(hasAgeForVoucher){
...
let howManyBoolean = 0;
let howManyNumber = 0;
let howManyString = 0;

for(let i = 0;i < crazyArray.length;i++){
  let isBoolean = typeof crazyArray[i] === 'boolean'
  let isNumber = typeof crazyArray[i] === 'number'
...
let msgTemplate = `O crazyArray tem ${howManyBoolean} booleans, ${howManyNumber} números e ${howManyString} strings.`
...
for(let i = 0;i < randomNumbers.length; i++){
  if(randomNumbers[i] % 2 === 0){
    numbersEven.push(randomNumbers[i])
    if(numbersEven.length === 3){
      evenString+="e " + randomNumbers[i]
    }
    else{
      evenString+=randomNumbers[i]+" "
    }
  }
  else{
    numbersOdd.push(randomNumbers[i])
    if(numbersOdd.length === 3){
      oddString+='e '+randomNumbers[i]
    }
    else{
      oddString+=randomNumbers[i]+" "
    }
  }
}

let msgTemplateB = `Numeros ímpares: ${oddString}. Números pares: ${evenString}.`
console.log(msgTemplateB)
