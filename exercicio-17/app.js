...
formElement.addEventListener('submit', event => {
  ...
  const inputText = event.target.input.value;
  const patternForm = /^[a-zA-Z0-9]{7,11}$/
  const isAValidInput = patternForm.test(inputText)
...
var pattern = /documentation/
var isAMatch = pattern.test(paragraph)
console.log(`Match: ${isAMatch}`)
...
pattern = /[B][9]{2}/
isAMatch = pattern.test(B99message)
console.log(`Match B99: ${isAMatch}`)
